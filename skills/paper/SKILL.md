...
写作和制图必须遵守仓库根目录 `PAPER_STYLE_GUIDE.md`。在引用 Freeze 前还要执行“替代定义挑战”：检查事件定义、观察机制或评价指标的合理替代是否会实质改变答案；若会，退回模型 Gate，禁止靠文字润色掩盖方法分歧。
...
Reviewer 必须按 `PAPER_STYLE_GUIDE.md` 分开评价内容充分性与视觉质量，并核对图表数量是否由论证需要决定。不得机械规定数量，但缺少支撑关键判断的诊断图、敏感性图或比较表时必须退回修改。
