...
正式参数研究的实验计划、验证证据、验证报告、冻结清单及审计报告会纳入版本控制；批量运行产生的各实验目录仍默认忽略，以免误提交大体积或含原始数据派生信息的结果。

论文目录默认跟踪 Markdown、LaTeX 与 JSON 审计 artifact；渲染产生的大型文件仍由作者确认后再决定是否提交。

`workspace/figures/` 中的最终 PNG 图像会纳入版本控制；逐页预览和其他临时渲染文件不应放入该目录。
